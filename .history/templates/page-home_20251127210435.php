 ...
 * Description: Template customizado para pÃ¡gina inicial - Parte 2 inserida
 * @package Camisa10
 * @version 2.0
 */

// ============================================
// FUNÃ‡ÃƒO HELPER PARA PREVENIR ARRAY TO STRING
// ============================================
...
<!-- ============================================
     ...
     Componentes: NÃºmero/Ãcone, TÃ­tulo, DescriÃ§Ã£o
     ============================================ -->
        ...
        <div class="row g-4 position-relative">
            ...
            <div class="col-12 col-md-6 col-lg-3">
                <div class="step-card text-center animate-on-scroll">
                    ...
                    <p class="step-description">
                        Navegue por nossa biblioteca com centenas de cursos em diversas Ã¡reas
                    </p>
                </div>
            </div>
            ...
            <div class="col-12 col-md-6 col-lg-3">
                <div class="step-card text-center animate-on-scroll">
                    ...
                    <p class="step-description">
                        Escolha o curso ideal e faÃ§a sua inscriÃ§Ã£o de forma rÃ¡pida e segura
                    </p>
                </div>
            </div>
            ...
            <div class="col-12 col-md-6 col-lg-3">
                <div class="step-card text-center animate-on-scroll">
                    ...
                    <p class="step-description">
                        Assista Ã s aulas no seu ritmo e pratique com exercÃ­cios reais
                    </p>
                </div>
            </div>
        ...
        <div class="row g-4 text-center">
            ...
            <div class="col-6 col-md-3">
                <div class="stat-card animate-on-scroll">
                    ...
                    <div class="stat-label">Cursos DisponÃ­veis</div>
                </div>
            </div>

            <!-- EstatÃ­stica 4: Rating -->
            <div class="col-6 col-md-3">
                <div class="stat-card animate-on-scroll">
                    <div class="stat-icon">
...
<!-- ============================================
     CURSOS EM DESTAQUE (VERSÃƒO FINAL BLINDADA)
     ============================================ -->
            ...
            try {
                ...
                if ($featured_courses->have_posts()) :
                    while ($featured_courses->have_posts()) : $featured_courses->the_post();
                        ...
                        // INICIALIZAR COM VALORES PADRÃƒO SEGUROS
                        $carga_horaria = '40h';
                        $formato = 'online';
                        $preco = 0;
                        $preco_promocional = 0;
...
// PEGAR VALORES COM VALIDAÃ‡ÃƒO DE TIPO (ACF PRO)
try {
    // ============================================
    // CARGA HORÃRIA
    // ============================================
    $temp_carga = get_field('curso_carga_horaria');
    if (is_array($temp_carga)) {
        // Se for array (select, taxonomy), pegar primeiro valor
        $carga_horaria = !empty($temp_carga) ? (string)reset($temp_carga) : $carga_horaria;
    } else {
    ...
    if (is_array($temp_formato)) {
        // Se for array, pegar 'value' ou primeiro item
        if (isset($temp_formato['value'])) {
            $formato = (string)$temp_formato['value'];
        } elseif (isset($temp_formato['label'])) {
            $formato = (string)$temp_formato['label'];
        } else {
            $formato = !empty($temp_formato) ? (string)reset($temp_formato) : $formato;
        }
    } else {
    ...
    if (is_array($temp_preco)) {
        $preco = 0; // PreÃ§o nÃ£o deveria ser array
        error_log('AVISO: campo curso_preco retornou array no post ' . get_the_ID());
    } else {
    ...
    if (is_array($temp_preco_promo)) {
        $preco_promocional = 0;
        error_log('AVISO: campo curso_preco_promocional retornou array no post ' . get_the_ID());
    } else {
    ...
    if (is_array($temp_nivel)) {
        // ACF Select pode retornar array com value/label
        if (isset($temp_nivel['value'])) {
            $nivel = (string)$temp_nivel['value'];
        } elseif (isset($temp_nivel['label'])) {
            $nivel = (string)$temp_nivel['label'];
        } else {
            $nivel = !empty($temp_nivel) ? (string)reset($temp_nivel) : $nivel;
        }
    } else {
    ...
    if (is_array($temp_rating)) {
        $rating = 5.0; // Default
        error_log('AVISO: campo curso_rating retornou array no post ' . get_the_ID());
    } else {
    ...
    if (is_array($temp_alunos)) {
        $total_alunos = 0;
        error_log('AVISO: campo curso_total_alunos retornou array no post ' . get_the_ID());
    } else {
        $total_alunos = !empty($temp_alunos) ? intval($temp_alunos) : 0;
    }

} catch (Exception $e) {
    error_log('âŒ Erro ao pegar campos ACF do curso ID ' . get_the_ID() . ': ' . $e->getMessage());
}
                        // Categorias com validaÃ§Ã£o
            ...
            <?php
                ...
                error_log('ERRO na seÃ§Ã£o de cursos: ' . $e->getMessage());
            ?>
                <div class="col-12">
...
<!-- ============================================
     ...
     Card: Ãcone, Nome, Contador, Link
     ============================================ -->
    ...
    <div class="container">
        ...
        <div class="row g-4">
            ...
            $categories = array(
                ...
                array('icon' => 'fa-chart-line', 'name' => 'NegÃ³cios', 'count' => 32, 'slug' => 'negocios'),
                array('icon' => 'fa-palette', 'name' => 'Design', 'count' => 28, 'slug' => 'design'),
                array('icon' => 'fa-bullhorn', 'name' => 'Marketing', 'count' => 24, 'slug' => 'marketing'),
                array('icon' => 'fa-users-cog', 'name' => 'GestÃ£o', 'count' => 18, 'slug' => 'gestao'),
...
<!-- ============================================
     ...
     Layout: Carousel com 3 cards visÃ­veis
     Componentes: Texto, Rating, Avatar, Nome, OcupaÃ§Ã£o
     ============================================ -->
<section id="depoimentos" class="testimonials-section section-padding bg-light">
    <div class="container">
        ...
        <div class="section-header text-center mb-5">
            ...
            <p class="section-subtitle">HistÃ³rias reais de transformaÃ§Ã£o profissional</p>
        </div>
        ...
        <div class="testimonials-slider">
            ...
            ?>
                <!-- Depoimentos de exemplo caso nÃ£o haja posts -->
                <div class="testimonial-card">
                    <div class="testimonial-quote-icon">
                        <i class="fas fa-quote-left"></i>
                    </div>
...
<!-- ============================================
     ...
     Card: Imagem, Nome, Especialidade, Stats, BotÃµes Social
     ============================================ -->
<section id="especialistas" class="instructors-section section-padding">
    <div class="container">
        ...
        <div class="section-header text-center mb-5">
            ...
            <p class="section-subtitle">Aprenda com profissionais reconhecidos que dominam suas Ã¡reas</p>
        </div>
...
<!-- ============================================
     CTA SECTION
     Layout: Centralizado
     Componentes: Headline, Subheadline, 2 BotÃµes, Background Destacado
     ============================================ -->
            ...
            <div class="col-12 col-lg-10 text-center">
                ...
                <div class="cta-buttons animate-on-scroll">
                    <a href="<?php echo esc_url(get_theme_mod('camisa10_cta_button1_url', '#')); ?>" class="btn btn-dark btn-lg me-3">
                        <?php echo get_theme_mod('camisa10_cta_button1_text', 'ComeÃ§ar Gratuitamente'); ?>
                    </a>
...
<!-- ============================================
     BLOG (ÃšLTIMAS NOTÃCIAS)
     Layout: Grid 3 colunas
     Card: Imagem, Categoria, Data, TÃ­tulo, Excerpt, Link
     ============================================ -->
<section id="blog" class="blog-section section-padding">
    <div class="container">
        <!-- CabeÃ§alho -->
        <div class="section-header text-center mb-5">
            <h2 class="section-title animate-on-scroll">Ãšltimas do Blog</h2>
...
<section id="faq" class="faq-section section-padding bg-light">
    <div class="container">
        ...
        <div class="section-header text-center mb-5">
            ...
            <p class="section-subtitle">Tire suas dÃºvidas sobre a plataforma e os cursos</p>
        </div>
                    ...
                    <div class="accordion-item animate-on-scroll">
                        <h2 class="accordion-header" id="faqHeading1">
                            <button class="accordion-button" type="button" data-bs-toggle="collapse" 
                                ...
                                Como faÃ§o para me inscrever em um curso?
                            </button>
                        </h2>
                        <div id="faqCollapse1" class="accordion-collapse collapse show" 
                            ...
                            <div class="accordion-body">
                                Para se inscrever, navegue atÃ© a pÃ¡gina do curso desejado e clique no botÃ£o "Inscrever-se Agora". 
                                VocÃª precisarÃ¡ criar uma conta ou fazer login caso jÃ¡ tenha uma. ApÃ³s o pagamento, vocÃª terÃ¡ acesso 
                                imediato ao conteÃºdo do curso.
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="accordion-item animate-on-scroll">
                        <h2 class="accordion-header" id="faqHeading2">
                            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" 
                                ...
                                Os cursos tÃªm certificado?
                            </button>
                        </h2>
                        <div id="faqCollapse2" class="accordion-collapse collapse" 
                             aria-labelledby="faqHeading2" data-bs-parent="#faqAccordionLeft">
                            <div class="accordion-body">
                                Sim! Todos os nossos cursos oferecem certificado de conclusÃ£o reconhecido no mercado. VocÃª receberÃ¡ 
                                o certificado automaticamente apÃ³s concluir todas as aulas e atividades do curso, com nota mÃ­nima de 
                                aprovaÃ§Ã£o de 70%.
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="accordion-item animate-on-scroll">
                        <h2 class="accordion-header" id="faqHeading3">
                            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" 
                                ...
                                Qual a duraÃ§Ã£o dos cursos?
                            </button>
                        </h2>
                        <div id="faqCollapse3" class="accordion-collapse collapse" 
                             aria-labelledby="faqHeading3" data-bs-parent="#faqAccordionLeft">
                            <div class="accordion-body">
                                A duraÃ§Ã£o varia de acordo com o curso. Temos cursos rÃ¡pidos de 2-5 horas, cursos mÃ©dios de 10-20 horas 
                                e cursos completos com mais de 30 horas de conteÃºdo. Todas as informaÃ§Ãµes estÃ£o disponÃ­veis na pÃ¡gina de 
                                cada curso.
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="accordion-item animate-on-scroll">
                        <h2 class="accordion-header" id="faqHeading4">
                            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" 
                                ...
                                Posso assistir as aulas no meu prÃ³prio ritmo?
                            </button>
                        </h2>
                        <div id="faqCollapse4" class="accordion-collapse collapse" 
                             aria-labelledby="faqHeading4" data-bs-parent="#faqAccordionLeft">
                            <div class="accordion-body">
                                Sim! Nossa plataforma Ã© 100% flexÃ­vel. VocÃª pode assistir Ã s aulas quando e onde quiser, quantas vezes 
                                precisar. NÃ£o hÃ¡ prazos rÃ­gidos - vocÃª aprende no seu prÃ³prio ritmo, de acordo com sua disponibilidade.
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="accordion-item animate-on-scroll">
                        <h2 class="accordion-header" id="faqHeading5">
                            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" 
                                ...
                                Existe alguma garantia de devoluÃ§Ã£o?
                            </button>
                        </h2>
                        <div id="faqCollapse5" class="accordion-collapse collapse" 
                             aria-labelledby="faqHeading5" data-bs-parent="#faqAccordionLeft">
                            <div class="accordion-body">
                                Sim! Oferecemos garantia de 7 dias. Se vocÃª nÃ£o estiver satisfeito com o curso por qualquer motivo, 
                                basta solicitar o reembolso total dentro deste perÃ­odo. Sem perguntas, sem complicaÃ§Ãµes.
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="accordion" id="faqAccordionRight">
                    ...
                    <div class="accordion-item animate-on-scroll">
                        ...
                        <div id="faqCollapse6" class="accordion-collapse collapse" 
                            ...
                            <div class="accordion-body">
                                Nossas mentorias sÃ£o sessÃµes individuais com especialistas. VocÃª agenda um horÃ¡rio, prepara suas dÃºvidas 
                                e objetivos, e participa de uma videoconferÃªncia exclusiva. O mentor vai guiar vocÃª com feedback 
                                personalizado e plano de aÃ§Ã£o especÃ­fico para suas necessidades.
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="accordion-item animate-on-scroll">
                        <h2 class="accordion-header" id="faqHeading7">
                            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" 
                                ...
                                Os cursos sÃ£o atualizados?
                            </button>
                        </h2>
                        <div id="faqCollapse7" class="accordion-collapse collapse" 
                             aria-labelledby="faqHeading7" data-bs-parent="#faqAccordionRight">
                            <div class="accordion-body">
                                Sim! Mantemos nossos cursos sempre atualizados com as Ãºltimas tendÃªncias e tecnologias do mercado. 
                                Quando vocÃª se inscreve em um curso, tem acesso vitalÃ­cio a todas as atualizaÃ§Ãµes futuras, sem custo 
                                adicional.
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="accordion-item animate-on-scroll">
                        ...
                        <div id="faqCollapse8" class="accordion-collapse collapse" 
                            ...
                            <div class="accordion-body">
                                Sim! Nossa plataforma Ã© 100% responsiva e funciona perfeitamente em celulares, tablets e computadores. 
                                VocÃª pode comeÃ§ar uma aula no computador e continuar no celular sem perder o progresso.
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="accordion-item animate-on-scroll">
                        <h2 class="accordion-header" id="faqHeading9">
                            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" 
                                ...
                                Preciso ter conhecimento prÃ©vio?
                            </button>
                        </h2>
                        <div id="faqCollapse9" class="accordion-collapse collapse" 
                             aria-labelledby="faqHeading9" data-bs-parent="#faqAccordionRight">
                            <div class="accordion-body">
                                Depende do curso! Cada curso indica claramente o nÃ­vel de conhecimento necessÃ¡rio: iniciante, 
                                intermediÃ¡rio ou avanÃ§ado. Temos cursos para todos os nÃ­veis, desde quem estÃ¡ comeÃ§ando do zero atÃ© 
                    ...
                    <div class="accordion-item animate-on-scroll">
                        ...
                        <div id="faqCollapse10" class="accordion-collapse collapse" 
                            ...
                            <div class="accordion-body">
                                VocÃª pode entrar em contato atravÃ©s do email suporte@camisa10.com.br, pelo chat ao vivo disponÃ­vel 
                                no canto inferior direito da tela, ou pelo WhatsApp (21) 99999-9999. Nossa equipe responde em atÃ© 24 
                                horas Ãºteis.
                            </div>
                        </div>
                    </div>
        ...
        <!-- Ainda tem dÃºvidas? -->
        <div class="text-center mt-5">
            <p class="lead mb-4">Ainda tem dÃºvidas?</p>
