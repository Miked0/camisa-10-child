...
function camisa10_single_curso_assets() {
    ...
    // 1. BOOTSTRAP CSS (se não estiver carregado)
    if (!wp_style_is('bootstrap', 'enqueued')) {
        wp_enqueue_style(
            'bootstrap',
            'https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css',
    ...
    // 4. CSS DO CURSO
    $curso_css_path = get_stylesheet_directory() . '/assets/css/single-curso.css';
    
    if (file_exists($curso_css_path)) {
        wp_enqueue_style(
            'camisa10-curso-css',
            get_stylesheet_directory_uri() . '/assets/css/single-curso.css',
            array('camisa10-variables', 'font-awesome'),
            filemtime($curso_css_path)
        );
    } else {
        // CSS INLINE TEMPORÁRIO se arquivo não existir
        add_action('wp_head', 'camisa10_curso_fallback_css');
    }
    ...
    // 6. JS DO CURSO
    $curso_js_path = get_stylesheet_directory() . '/assets/js/single-curso.js';
    
    if (file_exists($curso_js_path)) {
        wp_enqueue_script(
            'camisa10-curso-js',
            get_stylesheet_directory_uri() . '/assets/js/single-curso.js',
            array('jquery', 'bootstrap'),
            filemtime($curso_js_path),
            true
        );
    }
}
