 ...
 * - FIX #1: FunÃ§Ã£o camisa10_single_curso_assets() reimplementada (CRÃTICO)
 * - FIX #2: Bootstrap sempre registrado para dependÃªncias (CRÃTICO)
 * - FIX #3: filemtime() substituindo time() para cache correto (ALTO)
 * - FIX #4: cursos-section.css adicionado ao enqueue (MÃ‰DIO)
 * - FIX #5: Debug condicional para diagnÃ³stico (opcional, comentado)
 */
...
/**
 * ============================================
 * ASSETS ESPECÃFICOS DA HOMEPAGE
 * ============================================
 */
function camisa10_home_assets() {
    ...
    wp_enqueue_style(
        ...
        array('bootstrap'), // Agora dependÃªncia sempre satisfeita
 ...
 * ============================================
 * FIX #1: FunÃ§Ã£o estava ausente no arquivo original
 * FIX #3: time() substituÃ­do por filemtime() para cache correto
...
function camisa10_single_curso_assets() {
    ...
    /* FIX #5: DEBUG CONDICIONAL (opcional - descomentar para diagnÃ³stico)
    if (current_user_can('administrator')) {
        ...
        error_log('Bootstrap carregado: ' . (wp_style_is('bootstrap', 'enqueued') ? 'SIM' : 'NÃƒO'));
    }
    */

    /* FIX #3: VersÃµes calculadas UMA VEZ com filemtime()
     * ANTES: $timestamp = time(); (mudava a cada segundo)
     * DEPOIS: filemtime() (muda apenas quando arquivo Ã© editado)
     */
    ...
    // 2. CSS VARIABLES (dependÃªncia satisfeita)
    wp_enqueue_style(
        'camisa10-variables',
        get_stylesheet_directory_uri() . '/assets/css/custom-variables.css',
        array('bootstrap'),
        $vars_version // â† Agora usa filemtime() ao invÃ©s de time()
    );
    ...
    // 4. FIX #4: CURSOS SECTION CSS (arquivo estava Ã³rfÃ£o)
    wp_enqueue_style(
    ...
    wp_enqueue_style(
        ...
        $curso_version // â† Agora usa filemtime() ao invÃ©s de time()
    );
...
/**
 * ============================================
 * CORREÃ‡ÃƒO: ERRO DEPRECATED DO AVATAR
 * ============================================
 */
...
function camisa10_register_menus() {
    register_nav_menus(array(
        ...
        'footer-nav' => __('Menu Footer - NavegaÃ§Ã£o', 'camisa10'),
        'footer-content' => __('Menu Footer - ConteÃºdo', 'camisa10'),
    ));
}
...
function camisa10_hotmart_webhook_listener() {
    ...
    if (empty($hotmart_token)) {
        error_log('[Camisa10 Webhook] ERRO: Token nÃ£o configurado');
        http_response_code(500);
        exit('Webhook token not configured');
    }

    if ($received_token !== $hotmart_token) {
        error_log('[Camisa10 Webhook] ALERTA: Token invÃ¡lido');
